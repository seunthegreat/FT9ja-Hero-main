...
const HistoryModal = ({type}) => {
  ...
  return (
      ...
      >
          {type && type == 'payment' &&  (
              <div className='flex flex-col rounded-[10px] bg-white w-[50%] overflow-hidden p-10'>
                  <div className='flex flex-row md:mb-10 sm:mb-5 justify-between'>
                  ...
                  <div className='flex flex-col bg-gray-100 w-full justify-center py-10 px-5 items-center'>
                      <div className='grid grid-flow-col grid-cols-3 border-b-2 border-gray-200 w-full mb-5'>
                          <div className='md:p-3 sm:mb-2 sm:px-2'>
                              <p className={`md:text-xl sm:text-base font-normal`}>Date</p>
                          </div>
                          <div className='md:p-3 sm:mb-2 sm:px-2'>
                              <p className={`text-xl sm:text-base font-normal`}>Status</p>
                          </div>
                          <div className='md:p-3 sm:mb-2 sm:px-2'>
                              <p className={`text-xl sm:text-base font-normal`}>Payment Amount</p>
                          </div>
                      </div>
                      ...
                      {paymentHistory.map((item, index) => (
                          <div key={index} className='grid grid-flow-col grid-cols-3 border-b-2 border-gray-200 w-full'>
                              <div className='p-3'>
                                  <p className={`md:text-ss sm:text-xs font-light text-gray-500`}>{formatDate(item.date)}</p>
                              </div>
                              <div className='p-3 flex flex-row'>
                                  <div className={`${item.status == 'approved' && 'bg-fadeGreen' || item.status == 'declined' && 'bg-red-200'
                                      ...
                                      <p className={`${item.status == 'approved' ? 'text-[#008000]' : 'text-black'} text-ss sm:text-xs font-light text-gray-500`}>{item.status}</p>
                                  </div>
                              </div>
                              <div className='p-3'>
                                  <p className={`text-ss font-light sm:text-xs font-light text-gray-500`}>{addCommas(item.amount)}</p>
                              </div>
                          </div>
                      ))}
